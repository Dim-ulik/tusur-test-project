...
class block_equation extends block_base {
    ...
    function solver($a, $b, $c) {
        ...
        $DB->insert_record('block_equation', $record);
    }
    ...
    public function get_content() {
        ...
        $sql = "SELECT * FROM mdl_block_equation ORDER BY id DESC LIMIT 1" ;
    ...
    <script>
        ...
        function check() {
            let flag = 0;
            if (String(parseFloat(a_elem.value, 10)) !== String(a_elem.value)) {
                flag = 1;
            }
        ...
        $this->content->text .= "<div><a href='local/pages/mypage.php' class='a-class'>Показать историю решений</a></div>";
        $this->content->text .= "</div>";
    }
}
